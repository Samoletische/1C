...
Процедура СформироватьЖурнал()
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	ИТС_ПриказыНаЦены.Контрагент.ИТС_Затраты КАК Затраты,
	                      ...
	                      |	ИТС_ПриказыНаЦены.ДатаОкончания,
	                      |	ИТС_ПриказыНаЦены.ТорговыйПредставитель,
	                      |	ИТС_ПриказыНаЦены.Контрагент.ИТС_Затраты,
	                      |	0,
	                      ...
	                      |		ПО ИТС_ПриказыНаЦены.Номенклатура = ПартииТоваровНаСкладахОстатки.Номенклатура
	                      |ГДЕ
	...
	Для Каждого Строка Из ТЗ Цикл
		Если Строка.Определитель = 1 Тогда
			...
			Для Каждого Стр Из МассивСтрок Цикл
				Себестоимость = Стр.ЦенаВходящая;
				ЦенаДействующая = Стр.ЦенаДействующая;
				ТЗ.Удалить(Стр);
			КонецЦикла;
			Строка.ЦенаВходящая = ?(Себестоимость = 0, Строка.ЦенаВходящая, Себестоимость);
