...
Функция ОбработкаТЗ(ТЗ)
	...
	Для Каждого Строка Из ТЗ Цикл
		Если Строка.Определитель = 1 Тогда
			ПараметрыОтбора = Новый Структура("УникальныйИдентификатор, Номенклатура, Определитель", Строка.УникальныйИдентификатор, Строка.Номенклатура, 0);
			...
			Для Каждого Стр Из МассивСтрок Цикл
				...
				ТЗ.Удалить(Стр);
			КонецЦикла;
			СредняяЦена = ?(КоличествоОстаток = 0, 0, ОпределитьСреднююЦену(Строка.Номенклатура, КоличествоОстаток));
...
Функция СформироватьЦены(Данные)
	...
	ДатаОкончанияДокумента = НачалоДня(КонецГода(ТекущаяДата()));
	СтарыеУсловия = Неопределено;
	
	// узнаём дату начала и окончания, а так же ссылку последнего дополнительного условия
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ГДЕ
	                      |	НЕ УсловияПоставокПоДоговорамКонтрагентов.ПометкаУдаления
	                      |	И УсловияПоставокПоДоговорамКонтрагентов.Контрагент = &Контрагент
	...
	СтарыеУсловия = Запрос.Выполнить().Выгрузить();
	
	Попытка
		// сравниваем даты, чтобы понять сколько нужно делать новых условий, и нужно ли вообще
		Если СтарыеУсловия.Количество() = 0 Тогда
			СоздатьНовоеУсловие(Данные, Данные.ДатаНачала, ?(ЗначениеЗаполнено(Данные.ДатаОкончания), Данные.ДатаОкончания, ДатаОкончанияДокумента));
		Иначе
			сч = 0;
			Пока сч >= 0 Цикл
				Если СтарыеУсловия[сч].Ссылка.ДатаНачала <= Данные.ДатаНачала Тогда
					Если СтарыеУсловия[сч].Ссылка.ДатаНачала = Данные.ДатаНачала Тогда
						Если СтарыеУсловия[сч].Ссылка.ДатаОкончания = ?(ЗначениеЗаполнено(Данные.ДатаОкончания), Данные.ДатаОкончания, ДатаОкончанияДокумента) Тогда
							// проверено
							ДобавитьВУсловие(СтарыеУсловия[сч].Ссылка, Данные);
							Прервать;
						ИначеЕсли СтарыеУсловия[сч].Ссылка.ДатаОкончания < ?(ЗначениеЗаполнено(Данные.ДатаОкончания), Данные.ДатаОкончания, ДатаОкончанияДокумента) Тогда
							// проверено
							ДобавитьВУсловие(СтарыеУсловия[сч].Ссылка, Данные);
							Данные.ДатаНачала = КонецДня(СтарыеУсловия[сч].Ссылка.ДатаОкончания) + 1;
							сч = сч - 1;
							Если сч < 0 Тогда
								СоздатьНовоеУсловие(Данные, Данные.ДатаНачала, ?(ЗначениеЗаполнено(Данные.ДатаОкончания), Данные.ДатаОкончания, ДатаОкончанияДокумента));
							КонецЕсли;
						Иначе
							Копия = СоздатьКопиюПродлить(СтарыеУсловия[сч].Ссылка, КонецДня(?(ЗначениеЗаполнено(Данные.ДатаОкончания), Данные.ДатаОкончания, ДатаОкончанияДокумента)) + 1, СтарыеУсловия[сч].Ссылка.ДатаОкончания);
							ДобавитьВУсловие(СтарыеУсловия[сч].Ссылка, Данные);
							УменьшитьДатуОкончания(СтарыеУсловия[сч].Ссылка, ?(ЗначениеЗаполнено(Данные.ДатаОкончания), Данные.ДатаОкончания, ДатаОкончанияДокумента));
							Копия.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Неоперативный);
							Прервать;
						КонецЕсли;
					ИначеЕсли СтарыеУсловия[сч].Ссылка.ДатаОкончания < Данные.ДатаНачала Тогда
						// проверено
						сч = сч - 1;
						Если сч < 0 Тогда
							СоздатьНовоеУсловие(Данные, Данные.ДатаНачала, ?(ЗначениеЗаполнено(Данные.ДатаОкончания), Данные.ДатаОкончания, ДатаОкончанияДокумента));
					...
					ИначеЕсли СтарыеУсловия[сч].Ссылка.ДатаОкончания <= ?(ЗначениеЗаполнено(Данные.ДатаОкончания), Данные.ДатаОкончания, ДатаОкончанияДокумента) Тогда
						// проверено
						Копия = СоздатьКопиюДобавить(СтарыеУсловия[сч].Ссылка, Данные.ДатаНачала, СтарыеУсловия[сч].Ссылка.ДатаОкончания, Данные);
						ДатаОкончания = СтарыеУсловия[сч].Ссылка.ДатаОкончания;
						УменьшитьДатуОкончания(СтарыеУсловия[сч].Ссылка, НачалоДня(Данные.ДатаНачала - 1));
						Копия.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Неоперативный);
						Если ДатаОкончания = ?(ЗначениеЗаполнено(Данные.ДатаОкончания), Данные.ДатаОкончания, ДатаОкончанияДокумента) Тогда
							Прервать;
						КонецЕсли;
						Данные.ДатаНачала = КонецДня(ДатаОкончания) + 1;
						сч = сч - 1;
						Если сч < 0 Тогда
							СоздатьНовоеУсловие(Данные, Данные.ДатаНачала, ?(ЗначениеЗаполнено(Данные.ДатаОкончания), Данные.ДатаОкончания, ДатаОкончанияДокумента));
						КонецЕсли;
					Иначе
						// проверено
						Копия = СоздатьКопиюПродлить(СтарыеУсловия[сч].Ссылка, КонецДня(?(ЗначениеЗаполнено(Данные.ДатаОкончания), Данные.ДатаОкончания, ДатаОкончанияДокумента)) + 1, СтарыеУсловия[сч].Ссылка.ДатаОкончания);
						Копия2 = СоздатьКопиюДобавить(СтарыеУсловия[сч].Ссылка, Данные.ДатаНачала, ?(ЗначениеЗаполнено(Данные.ДатаОкончания), Данные.ДатаОкончания, ДатаОкончанияДокумента), Данные);
						УменьшитьДатуОкончания(СтарыеУсловия[сч].Ссылка, НачалоДня(Данные.ДатаНачала - 1));
						Копия2.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Неоперативный);
						Копия.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Неоперативный);
						Прервать;
					КонецЕсли;
				Иначе
					Если сч = СтарыеУсловия.Количество() - 1 Тогда
						Если СтарыеУсловия[сч].Ссылка.ДатаНачала <= ?(ЗначениеЗаполнено(Данные.ДатаОкончания), Данные.ДатаОкончания, ДатаОкончанияДокумента) Тогда
							СоздатьНовоеУсловие(Данные, Данные.ДатаНачала, НачалоДня(СтарыеУсловия[сч].Ссылка.ДатаНачала - 1));
							Данные.ДатаНачала = СтарыеУсловия[сч].Ссылка.ДатаНачала;
							Продолжить;
						Иначе
							СоздатьНовоеУсловие(Данные, Данные.ДатаНачала, ?(ЗначениеЗаполнено(Данные.ДатаОкончания), Данные.ДатаОкончания, ДатаОкончанияДокумента));
							Прервать;
						КонецЕсли;
					ИначеЕсли СтарыеУсловия[сч + 1].Ссылка.ДатаОкончания < Данные.ДатаНачала Тогда
						Если СтарыеУсловия[сч].Ссылка.ДатаНачала <= ?(ЗначениеЗаполнено(Данные.ДатаОкончания), Данные.ДатаОкончания, ДатаОкончанияДокумента) Тогда
							СоздатьНовоеУсловие(Данные, Данные.ДатаНачала, НачалоДня(СтарыеУсловия[сч].Ссылка.ДатаНачала - 1));
							Данные.ДатаНачала = СтарыеУсловия[сч].Ссылка.ДатаНачала;
							Продолжить;
...
Процедура УменьшитьДатуОкончания(Условие, ДатаОкончания)

	Документ = Условие.ПолучитьОбъект();
...
КонецПроцедуры // УменьшитьДатуОкончания

Функция СоздатьКопиюПродлить(Условие, ДатаНачала, ДатаОкончания)

	// создаём документ
	докУсловиеПоставок = Документы.УсловияПоставокПоДоговорамКонтрагентов.СоздатьДокумент();
	докУсловиеПоставок.Дата = ТекущаяДата();
	докУсловиеПоставок.Контрагент = Условие.Контрагент;
	докУсловиеПоставок.ДоговорКонтрагента = Условие.ДоговорКонтрагента;
	докУсловиеПоставок.ДатаНачала = ДатаНачала;
	докУсловиеПоставок.ДатаОкончания = ДатаОкончания;
	докУсловиеПоставок.СуммаУсловийДоговора = Условие.СуммаУсловийДоговора;
	докУсловиеПоставок.Комментарий = "Сформировано автоматически при утверждении приказа новой цены";
	
	// копируем номенклатуру со старого документа
	докУсловиеПоставок.НоменклатураДоговора.Загрузить(Условие.НоменклатураДоговора.Выгрузить());
	
	// возвращаем документ
	Возврат докУсловиеПоставок;

КонецФункции // СоздатьКопиюПродлить

Процедура ДобавитьВУсловие(Условие, Данные)

	Документ = Условие.ПолучитьОбъект();
	
	// ищем присутствует ли уже номенклатура в списке
	ПараметрыОтбора = Новый Структура("Номенклатура", Данные.Номенклатура);
	МассивСтрок = Документ.НоменклатураДоговора.НайтиСтроки(ПараметрыОтбора);
	
	Если МассивСтрок.Количество() = 0 Тогда
		Номенклатура = Документ.НоменклатураДоговора.Добавить();
	...
	Иначе
		МассивСтрок[0].Цена = Данные.ЦенаПредлагаемая;
	КонецЕсли;
...
Функция СоздатьКопиюДобавить(Условие, ДатаНачала, ДатаОкончания, Данные)
	...
	// ищем присутствует ли уже номенклатура в списке
	ПараметрыОтбора = Новый Структура("Номенклатура", Данные.Номенклатура);
	МассивСтрок = докУсловиеПоставок.НоменклатураДоговора.НайтиСтроки(ПараметрыОтбора);
...
КонецФункции // СоздатьКопиюДобавить

Процедура СоздатьНовоеУсловие(Данные, ДатаНачала, ДатаОкончания)
...
КонецПроцедуры // СоздатьНовоеУсловие
...
Процедура ЖурналПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если ЭлементыФормы.Журнал.ТекущаяКолонка.Имя = "ДатаНачала"
		ИЛИ ЭлементыФормы.Журнал.ТекущаяКолонка.Имя = "ДатаОкончания"
		ИЛИ ЭлементыФормы.Журнал.ТекущаяКолонка.Имя = "ЦенаПредлагаемая" Тогда
		...
		Если НаборЗаписей.Количество() > 0 Тогда
			...
			НаборЗаписей[0].Цена = ТекущиеДанные.ЦенаПредлагаемая;
			
			// вычисляем разницы цен
			ТекущиеДанные.РазницаМеждуНовойИДействующей = ТекущиеДанные.ЦенаДействующая - ТекущиеДанные.ЦенаПредлагаемая;
			ТекущиеДанные.РазницаМеждуНовойИВходЗатраты = ТекущиеДанные.ЦенаПредлагаемая - (ТекущиеДанные.ЦенаВходящая + ТекущиеДанные.Затраты);
		Иначе
			Сообщить("Нарушение целостности регистра. Запись не найдена.");
		КонецЕсли;
		НаборЗаписей.Записать();
